...
: Node("zed_cvt_node", options), mVideoQos(1)
{
  ...
  RCLCPP_INFO(get_logger(), " ZED BGRA2RGB Convert Component ");
  RCLCPP_INFO(get_logger(), "********************************");
  ...
  const sensor_msgs::msg::CameraInfo::ConstSharedPtr & cam_info)
{
  ...
  std::shared_ptr<sensor_msgs::msg::Image> out_rgb = std::make_shared<sensor_msgs::msg::Image>();

  out_rgb->header.stamp = img->header.stamp;
  out_rgb->header.frame_id = img->header.frame_id;
  out_rgb->height = rgb.rows;
  out_rgb->width = rgb.cols;

  int num = 1;  // for endianness detection
  out_rgb->is_bigendian = !(*reinterpret_cast<char *>(&num) == 1);

  out_rgb->step = rgb.step;

  size_t size = out_rgb->step * out_rgb->height;
  out_rgb->data.resize(size);

  out_rgb->encoding = sensor_msgs::image_encodings::RGB8;
  memcpy(reinterpret_cast<char *>((&out_rgb->data[0])), &rgb.data[0], size);

  // Publish the new image message coupled with camera info from the original message
  mPubBgr.publish(out_rgb, cam_info);
}
